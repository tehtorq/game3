...
impl Enemy {
    pub fn new(x: f32, z: f32, enemy_type: EnemyType) -> Self {
        Self::new_with_terrain(x, z, enemy_type, None)
    }
    
    pub fn new_with_terrain(x: f32, z: f32, enemy_type: EnemyType, terrain: Option<&InstancedTerrain>) -> Self {
        let mut rng = thread_rng();
        
        // Calculate spawn height based on terrain
        let terrain_height = if let Some(t) = terrain {
            t.get_height_at(x, z)
        } else {
        ...
        let patrol_waypoints = {
            ...
            for i in 0..waypoint_count {
                ...
                let wy = if let Some(t) = terrain {
                    t.get_height_at(wx, wz)
                } else {
                    terrain_height_fallback(wx, wz)
    ...
    pub fn update_with_player(&mut self, player_pos: Vec3, dt: f32) -> Option<Vec3> {
        ...
        match effective_pattern {
            MovementPattern::Hover => {
                ...
                let terrain_height = terrain_height_fallback(self.pos.x, self.pos.z);
            ...
            MovementPattern::Orbital => {
                ...
                let target_y = terrain_height_fallback(target_x, target_z) + 60.0 + (self.phase * 2.0).sin() * 20.0;
