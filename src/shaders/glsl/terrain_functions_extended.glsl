// Extended terrain generation functions for instanced terrain shader
// Includes all biomes
...
float get_biome_height_extended(vec2 p) {
...
float get_blended_biome_height_extended(vec2 p) {
    // Sample multiple nearby points for smoother transitions
    float sample_dist = 100.0; // Increased for larger features
    float h_center = get_biome_height_extended(p);
    float h_north = get_biome_height_extended(p + vec2(0.0, sample_dist));
    float h_south = get_biome_height_extended(p + vec2(0.0, -sample_dist));
    float h_east = get_biome_height_extended(p + vec2(sample_dist, 0.0));
    float h_west = get_biome_height_extended(p + vec2(-sample_dist, 0.0));
