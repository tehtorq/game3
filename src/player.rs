...
impl Player {
    ...
    pub fn update(&mut self, left: bool, right: bool, up: bool, down: bool, boost: bool, dt: f32, terrain: Option<&InstancedTerrain>) {
        // Physics constants
        ...
        let terrain_below = if let Some(terrain) = terrain {
            terrain.get_height_at(self.pos.x, self.pos.z)
        } else {
            // Fallback that better approximates the shader's terrain
            terrain_height_fallback(self.pos.x, self.pos.z)
        };
    ...
    pub fn get_terrain_height(&self, terrain: Option<&InstancedTerrain>) -> f32 {
    ...
    pub fn get_height_above_terrain(&self, terrain: Option<&InstancedTerrain>) -> f32 {
        self.pos.y - self.get_terrain_height(terrain)
    }
