...
pub struct DamageShieldEffect {
    ...
    shield_intensity: f32,
}

impl PostProcessingEffect for DamageShieldEffect {
    fn new(ctx: &mut Context, screen_width: u32, screen_height: u32) -> Self {
        ...
        let shader = ctx.new_shader(
            ShaderSource::Glsl {
                vertex: shaders::modules::postfx::damage_shield::VERTEX,
                fragment: shaders::modules::postfx::damage_shield::FRAGMENT,
            },
            ShaderMeta {
                ...
                uniforms: UniformBlockLayout {
                    uniforms: vec![
                        ...
                        UniformDesc::new("u_damage_intensity", UniformType::Float1),
                        UniformDesc::new("u_shield_intensity", UniformType::Float1),
                        UniformDesc::new("u_time", UniformType::Float1),
                    ],
                },
            }
        ).expect("Failed to create damage/shield shader");
    ...
    fn toggle(&mut self) {
        ...
        println!("Damage/Shield Effect: {}", if self.enabled { "ON" } else { "OFF" });
    }
    ...
    fn apply_effect(
        ...
        let uniforms = Uniforms {
            ...
            damage_intensity: self.damage_intensity,
            shield_intensity: self.shield_intensity,
            time: 0.0,
        };
...
impl DamageShieldEffect {
    pub fn update_from_player_health(&mut self, health: f32, shield: f32) {
        // Damage effect increases as health decreases
        self.damage_intensity = if health < 0.3 { 
            (0.3 - health) / 0.3 
        } else { 
            0.0 
        };
        
        // Shield effect shows when shield is active
        self.shield_intensity = shield;
        
        // Auto-enable when there's damage or shield
        if self.damage_intensity > 0.0 || self.shield_intensity > 0.0 {
            self.enabled = true;
        }
    }
}
