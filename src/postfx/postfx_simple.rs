...
// Simple chromatic aberration post-processing example
...
impl PostProcessingEffect for ChromaticAberrationEffect {
    fn new(ctx: &mut Context, screen_width: u32, screen_height: u32) -> Self {
        ...
        let shader = ctx.new_shader(
            ShaderSource::Glsl {
                vertex: shaders::modules::postfx::chromatic_aberration::VERTEX,
                fragment: shaders::modules::postfx::chromatic_aberration::FRAGMENT,
            },
            ShaderMeta {
                ...
                uniforms: UniformBlockLayout {
                    uniforms: vec![
                        ...
                        UniformDesc::new("u_aberration_strength", UniformType::Float1),
                        UniformDesc::new("u_speed_factor", UniformType::Float1),
                        UniformDesc::new("u_time", UniformType::Float1),
                    ],
                },
            }
        ).expect("Failed to create chromatic aberration shader");
    ...
    fn toggle(&mut self) {
        ...
        println!("Chromatic Aberration: {}", if self.enabled { "ON" } else { "OFF" });
    }
    ...
    fn apply_effect(
        ...
        let uniforms = Uniforms {
            ...
            speed_factor: 0.0, // These are not used in this effect
            time: 0.0,
        };
...
impl ChromaticAberrationEffect {
    pub fn increase_strength(&mut self) {
        self.aberration_strength = (self.aberration_strength + 0.005).min(0.1);
        println!("Chromatic Aberration strength: {:.3}", self.aberration_strength);
    }
    
    pub fn decrease_strength(&mut self) {
        self.aberration_strength = (self.aberration_strength - 0.005).max(0.0);
        println!("Chromatic Aberration strength: {:.3}", self.aberration_strength);
    }
}
