...
impl PostProcessingEffect for CrtEffect {
    fn new(ctx: &mut Context, screen_width: u32, screen_height: u32) -> Self {
        ...
        let shader = ctx.new_shader(
            ShaderSource::Glsl {
                vertex: shaders::modules::postfx::crt::VERTEX,
                fragment: shaders::modules::postfx::crt::FRAGMENT,
            },
            ShaderMeta {
                ...
                uniforms: UniformBlockLayout {
                    uniforms: vec![
                        ...
                        UniformDesc::new("u_curvature", UniformType::Float1),
                        UniformDesc::new("u_scanline_intensity", UniformType::Float1),
                        UniformDesc::new("u_time", UniformType::Float1),
                    ],
                },
            }
        ).expect("Failed to create CRT shader");
    ...
    fn toggle(&mut self) {
        ...
        println!("CRT Effect: {}", if self.enabled { "ON" } else { "OFF" });
    }
...
impl CrtEffect {
    pub fn increase_curvature(&mut self) {
        self.curvature = (self.curvature + 0.05).min(0.5);
        println!("CRT curvature: {:.2}", self.curvature);
    }
    
    pub fn decrease_curvature(&mut self) {
        self.curvature = (self.curvature - 0.05).max(0.0);
        println!("CRT curvature: {:.2}", self.curvature);
    }
}
