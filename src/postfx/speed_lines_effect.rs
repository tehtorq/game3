...
impl PostProcessingEffect for SpeedLinesEffect {
    fn new(ctx: &mut Context, screen_width: u32, screen_height: u32) -> Self {
        ...
        let shader = ctx.new_shader(
            ShaderSource::Glsl {
                vertex: shaders::modules::postfx::speed_lines::VERTEX,
                fragment: shaders::modules::postfx::speed_lines::FRAGMENT,
            },
            ShaderMeta {
                ...
                uniforms: UniformBlockLayout {
                    uniforms: vec![
                        ...
                        UniformDesc::new("u_line_count", UniformType::Float1),
                        UniformDesc::new("u_time", UniformType::Float1),
                    ],
                },
            }
        ).expect("Failed to create speed lines shader");
    ...
    fn toggle(&mut self) {
        ...
        println!("Speed Lines: {}", if self.enabled { "ON" } else { "OFF" });
    }
...
impl SpeedLinesEffect {
    pub fn increase_intensity(&mut self) {
        ...
        println!("Speed Lines intensity: {:.1}", self.intensity);
    }
