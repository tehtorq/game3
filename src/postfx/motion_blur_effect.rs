...
impl PostProcessingEffect for MotionBlurEffect {
    ...
    fn toggle(&mut self) {
        ...
        println!("Motion Blur: {}", if self.enabled { "ON" } else { "OFF" });
    }
    ...
    fn apply_effect(
        ...
        let uniforms = Uniforms {
            ...
            velocity: [0.0, 0.0], // This should be updated from the game state
...
impl MotionBlurEffect {
    pub fn increase_strength(&mut self) {
        self.blur_strength = (self.blur_strength + 0.1).min(2.0);
        println!("Motion Blur strength: {:.1}", self.blur_strength);
    }
    
    pub fn decrease_strength(&mut self) {
        self.blur_strength = (self.blur_strength - 0.1).max(0.0);
        println!("Motion Blur strength: {:.1}", self.blur_strength);
    }
}
